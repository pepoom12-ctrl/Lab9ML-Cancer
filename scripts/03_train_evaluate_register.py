...
def train_evaluate_register(preprocessing_run_id, C=10.0):
    ...
    MODEL_NAME = "cancer-classifier-prod"  # ตั้งชื่อโมเดลตามโจทย์
    ...
    with mlflow.start_run(run_name=f"logistic_regression_C_{C}"):
        ...
        # คำนวณทั้ง Accuracy และ ROC-AUC
        ...
        model_info = mlflow.sklearn.log_model(
            sk_model=pipeline,
            name="breast_classifier_pipeline",
            input_example=X_train.head(5)
        )

        # Quality Gate ต้องผ่านทั้งสองเกณฑ์
        if acc >= ACCURACY_THRESHOLD and roc_auc >= ROC_AUC_THRESHOLD:
            print("Model met quality gate criteria. Registering model...")
            ...
            print(f"Registered model as version {registered_model.version} with alias @staging")
        else:
            print("Model did not meet requirements. Not registering.")
...
if __name__ == "__main__":
    ...
    run_id = sys.argv[1]
    c_value = float(sys.argv[2]) if len(sys.argv) > 2 else 10.0
